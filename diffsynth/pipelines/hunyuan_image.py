...
class ImageSizeManager:
    ...
    def get_fill_resize_and_crop(self, src, tgt):
        ...
        if r > tr:
            ...
            resize_width = tw
            resize_height = int(round(tw / w * h))    # Resize the target resolution according to the base resolution.
    ...
    def get_meshgrid(self, start, *args):
        if len(args) == 0:
            ...
            start = self._to_tuple(start)       # Top-left corner, e.g. (12, 0).
            stop = self._to_tuple(args[0])      # Bottom-right corner, e.g. (20, 32).
            num = self._to_tuple(args[1])       # Target size, e.g. (32, 124).
        else:
            raise ValueError(f"len(args) should be 0, 1 or 2, but got {len(args)}")

        grid_h = np.linspace(start[0], stop[0], num[0], endpoint=False, dtype=np.float32) # Divide the 12–20 and 0–32 intervals into 32 and 124 parts, respectively.
    ...
    def get_2d_rotary_pos_embed(self, embed_dim, start, *args, use_real=True):
        ...
        grid = grid.reshape([2, 1, *grid.shape[1:]])   # Return a sampling matrix at the target resolution.

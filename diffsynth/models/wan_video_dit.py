...
from .wan_video_segmentation import SemanticFPNHead
try:
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, ip_kv: Optional[Dict[str, torch.Tensor]] = None):
        """
        Forward pass with optional IP-Adapter injection.
        
        Args:
            x: Query tensor (B, L, D), latent features
            y: Context tensor (B, L_ctx, D), text features
            ip_kv: Optional IP-Adapter K, V dict with keys "ip_k", "ip_v", "scale"
                  ip_k: (B, num_heads, num_tokens, head_dim)
                  ip_v: (B, num_heads, num_tokens, head_dim)
                  scale: float
        """
        ...
        if ip_kv is not None:
            ip_k = ip_kv.get("ip_k")
            ip_v = ip_kv.get("ip_v")
            ip_scale = ip_kv.get("scale", 1.0)
            if ip_k is not None and ip_v is not None:
                # Reshape q for attention: (B, L, D) -> (B, num_heads, L, head_dim)
                B, L, D = q.shape
                q_ip = q.view(B, L, self.num_heads, self.head_dim).transpose(1, 2)  # (B, num_heads, L, head_dim)
                # Apply IP-Adapter attention
                # ip_k: (B, num_heads, num_tokens, head_dim)
                # ip_v: (B, num_heads, num_tokens, head_dim)
                # Compute attention scores: (B, num_heads, L, num_tokens)
                attn_scores = torch.matmul(q_ip, ip_k.transpose(-2, -1)) / (self.head_dim ** 0.5)
                attn_weights = torch.softmax(attn_scores, dim=-1)
                # Apply attention: (B, num_heads, L, head_dim)
                ip_out = torch.matmul(attn_weights, ip_v)
                # Reshape back: (B, num_heads, L, head_dim) -> (B, L, D)
                ip_out = ip_out.transpose(1, 2).contiguous().view(B, L, D)
                x = x + ip_out * ip_scale
        return self.o(x)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, ip_kv: Optional[Dict[str, torch.Tensor]] = None):
        """
        Forward pass with optional IP-Adapter injection.
        ...
        Args:
            ...
            ip_kv: Optional IP-Adapter K, V dict
        """
        ...
        x = x + self.cross_attn(self.norm3(x), context, ip_kv=ip_kv)
...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        use_perception_head: bool = False,
        ...
        # Segmentation head for latent segmentation
        if use_perception_head:
            self.start_layer = 0
            self.end_layer = num_layers - 1
            self.perception_head = SemanticFPNHead(
                in_channels=dim,
                out_channels=in_dim,  # Output channels for segmentation mask (match VAE channels)
        ...
        # Depth head for latent depth prediction
        if use_depth_head:
            if not hasattr(self, 'start_layer'):
                self.start_layer = 0
            if not hasattr(self, 'end_layer'):
                self.end_layer = num_layers - 1
            self.depth_head = SemanticFPNHead(
                in_channels=dim,
                out_channels=in_dim,  # Output channels for depth (match VAE channels)
                num_tensors=self.end_layer - self.start_layer + 1,
                patch_size=patch_size,
            )
        else:
    ...
    def forward(self,
        ...
        # Collect features from all transformer blocks for segmentation/depth (only if needed)
        diffusion_features = []
        for block in self.blocks:
            ...
            # Collect features if perception_head or depth_head is enabled
            if self.perception_head is not None or self.depth_head is not None:
        ...
        # Return mask_pred and/or depth_pred if available
        if mask_pred is not None and depth_pred is not None:

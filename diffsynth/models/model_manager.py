...
def load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device):
    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        if hasattr(model, "eval"):
            model = model.eval()
        allow_missing = getattr(model, "allow_missing_keys", False)
        model.load_state_dict(model_state_dict, assign=True, strict=not allow_missing)
        # Handle meta tensors (e.g., missing newly added params)
        has_meta = any(param.is_meta for param in model.parameters())
        if has_meta:
            model = model.to_empty(device=device)
            model.load_state_dict(model_state_dict, assign=True, strict=not allow_missing)
            if hasattr(model, "init_missing_params"):
                model.init_missing_params()
